...
//            p('事件菜单');
    ...
    {
        $article_num = $this->Article_model->get_count();
        $chat_num = $this->Chat_model->get_count();
        $tag_count = $this->Tag_model->get_count();
        $member_num = $this->Oauth_model->get_count();
        $comment_count = $this->Article_model->get_allcomment_count();

        $arr = [
            ['文章数量', (int)$article_num],
            ['评论数量',(int)$comment_count],
            ['会员数量',(int)$member_num],
            ['碎言数量', (int)$chat_num],
            ['标签数量', (int)$tag_count]
        ];
        $data['list'] = json_encode($arr);
        $this->load->view('index/index_new', $data);
    }
    ...
    public function check_auth()
    {
        ...
        $_ACCESS_LIST = $this->session->userdata('_ACCESS_LIST');//用户登录时保存在session中的权限,菜单权限
        $_ACTION_ACCESS_LIST = $this->session->userdata('_ACTION_ACCESS_LIST');//用户登录时保存在session中的权限,操作权限,用于判断是否有操作权限
        $_ACTION_ACCESS_LIST = explode(',', $_ACTION_ACCESS_LIST);
